...
function getNextFriday(date) {
  ...
  const fridayOrderNumber = 5;
  let daysCount;

  if (day > 5) {
    daysCount = fridayOrderNumber + (7 - day);
  } else if (day < 5) {
    daysCount = fridayOrderNumber - day;
  } else {
    daysCount = 7;
  }

  const nextFriday = new Date(parsedDate);
  nextFriday.setUTCDate(parsedDate.getUTCDate() + daysCount);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diffInMs = dateEndObj.getTime() - dateStartObj.getTime();
...
function isDateInPeriod(date, period) {
  ...
  const startInMs = new Date(period.start).getTime();
  const endInMs = new Date(period.end).getTime();
  return dateInMs >= startInMs && dateInMs <= endInMs;
}
